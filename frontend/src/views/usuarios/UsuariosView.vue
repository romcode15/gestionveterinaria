...
import { ROL_OPTIONS } from '@/constants/filterOptions'
...
const { busqueda, setFiltro } = useFiltros({
  onCargar: (page) => usuariosStore.cargar({ page }),
})

// Sincronizar busqueda → store
watch(busqueda, (val) => { usuariosStore.searchQuery = val })

onMounted(() => usuariosStore.cargar({ page: 0 }))

// Filtro de rol: respuesta inmediata
    ...
    <TableViewLayout>
      <template #toolbar>
        ...
        >
          <template #filters>
            <AppSelect
              :model-value="usuariosStore.filtroRol"
              :options="rolOptions"
              class="w-full sm:w-52"
              @update:model-value="(v) => { usuariosStore.filtroRol = v as string; setFiltro('rol', v as string) }"
            />
          </template>
        </SearchToolbar>
      </template>
          ...
          >
            <template #cell-usuario="{ row }">
              <div class="flex items-center gap-3">
                <div class="w-9 h-9 rounded-full flex items-center justify-center font-semibold text-sm shrink-0 text-white"
            ...
            <template #cell-roles="{ row }">
              <div class="flex flex-wrap gap-1 justify-center">
                <AppBadge v-for="rol in row.rolesNombres" :key="rol" :variant="rolVariant[rol] ?? 'neutral'" size="sm">
                  {{ rolLabels[rol] ?? rol }}
                </AppBadge>
              </div>
            </template>
            ...
            <template #actions="{ row }">
              <div class="flex items-center justify-end gap-1">
                ...
                <AppButton variant="danger" size="sm" title="Eliminar" @click.stop="confirmarEliminar(row)">
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
