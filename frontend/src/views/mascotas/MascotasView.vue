<script setup lang="ts">
import { ref, computed, watch, onMounted } from 'vue'

import DashboardLayout from '@/layouts/DashboardLayout.vue'

import AppCard from '@/components/ui/AppCard.vue'
import AppModal from '@/components/ui/AppModal.vue'
import AppSelect from '@/components/ui/AppSelect.vue'
import AppAlert from '@/components/ui/AppAlert.vue'
import AppButton from '@/components/ui/AppButton.vue'
import AppPagination from '@/components/ui/AppPagination.vue'
import AppTable from '@/components/ui/AppTable.vue'
import AppBadge from '@/components/ui/AppBadge.vue'

import PageHeader from '@/components/common/PageHeader.vue'
import SearchToolbar from '@/components/common/SearchToolbar.vue'
import EntityCard from '@/components/common/EntityCard.vue'
import EmptyState from '@/components/common/EmptyState.vue'
import TableViewLayout from '@/components/common/TableViewLayout.vue'

import MascotaAvatar from '@/components/mascotas/MascotaAvatar.vue'
import MascotaForm from '@/components/mascotas/MascotaForm.vue'

import { useMascotasStore } from '@/stores/mascotas.store'
import { useAuthStore } from '@/stores/auth.store'
import { clientesService } from '@/services/clientes.service'
import { useFiltros } from '@/composables/useFiltros'
import type { Mascota, MascotaFormData, Cliente } from '@/types'

// Columnas de la tabla de mascotas
const columns = [
  {
    key: 'mascota',
    label: 'Mascota',
  },
  {
    key: 'especie_raza',
    label: 'Especie / Raza',
  },
  {
    key: 'sexo',
    label: 'Sexo',
    align: 'center',
  },
  {
    key: 'edad',
    label: 'Edad',
    align: 'center',
  },
  {
    key: 'peso',
    label: 'Peso',
    align: 'center',
  },
  {
    key: 'propietario',
    label: 'Propietario',
  },
] as const

// Calcula edad de la mascota
function calcularEdad(fechaNacimiento?: string): string {
  if (!fechaNacimiento) return '-'
  const hoy = new Date()
  const nacimiento = new Date(fechaNacimiento)
  const años = hoy.getFullYear() - nacimiento.getFullYear()
  if (años === 0) {
    const meses = hoy.getMonth() - nacimiento.getMonth()
    return `${meses < 0 ? meses + 12 : meses}m`
  }
  return `${años}a`
}

const mascotasStore = useMascotasStore()
const authStore     = useAuthStore()

// Lista local de clientes solo para el dropdown del formulario
const clientesDropdown = ref<Cliente[]>([])

// Composable centralizado: debounce 400ms en texto, inmediato en selects
const { busqueda, setFiltro } = useFiltros({
  onCargar: (page) => mascotasStore.cargar({ page }),
})

// Sincronizar busqueda → store
watch(busqueda, (val) => { mascotasStore.searchQuery = val })

onMounted(async () => {
  mascotasStore.medicoId = authStore.isMedico ? (authStore.medicoId ?? null) : null
  await Promise.all([
    mascotasStore.cargar({ page: 0 }),
    mascotasStore.cargarCatalogos(),
    ...(authStore.isMedico ? [] : [
      clientesService.getAll({ size: 200, sort: 'apellido', dir: 'asc' })
        .then(res => { clientesDropdown.value = res.content })
    ]),
  ])
})

// Estado de la vista
const showModal = ref(false)
const mascotaEditando = ref<Mascota | null>(null)
const successMessage = ref('')
const loading = ref(false)
const vistaGrid = ref(false)

// Opciones del filtro de especies
const especieOptions = computed(() => [
  {
    value: '',
    label: 'Todas las especies',
  },
  ...mascotasStore.especies.map((e) => ({
    value: e.id,
    label: e.nombre,
  })),
])

// Abrir formulario nuevo
function abrirCrear() {
  mascotaEditando.value = null
  showModal.value = true
}

// Abrir formulario edición
function abrirEditar(mascota: Mascota) {
  mascotaEditando.value = mascota
  showModal.value = true
}

// Guardar mascota
async function handleSubmit(data: MascotaFormData) {
  loading.value = true
  try {
    if (mascotaEditando.value) {
      await mascotasStore.actualizar(mascotaEditando.value.id, data)
      successMessage.value = 'Mascota actualizada correctamente'
    } else {
      await mascotasStore.crear(data)
      successMessage.value = 'Mascota registrada correctamente'
    }
    showModal.value = false
    setTimeout(() => {
      successMessage.value = ''
    }, 3000)
  } finally {
    loading.value = false
  }
}
</script>

<template>
  <DashboardLayout>
    <template #header>
      <PageHeader title="Mascotas" subtitle="Gestión de pacientes veterinarios" />
    </template>

    <TableViewLayout>
      <template #toolbar>
        <Transition name="fade">
          <AppAlert v-if="mascotasStore.error" type="error" dismissible @dismiss="mascotasStore.limpiarError()">
            {{ mascotasStore.error }}
          </AppAlert>
        </Transition>
        <Transition name="fade">
          <AppAlert v-if="successMessage" type="success" dismissible @dismiss="successMessage = ''">
            {{ successMessage }}
          </AppAlert>
        </Transition>

        <SearchToolbar
          v-model:search="busqueda"
          search-placeholder="Buscar por nombre, especie, propietario..."
          :show-new-button="!authStore.isMedico"
          new-button-label="Nueva mascota"
          @new="abrirCrear"
        >
          <template #filters>
            <AppSelect
              :model-value="mascotasStore.filtroEspecieId ?? ''"
              :options="especieOptions"
              class="w-full sm:w-48"
              @update:model-value="(v) => {
                mascotasStore.filtroEspecieId = v ? Number(v) : null
                setFiltro('especieId', v ? String(v) : '')
              }"
            />
          </template>
          <template #actions>
            <div class="flex rounded-lg overflow-hidden border" style="border-color: var(--border-color)">
              <button
                :class="['px-3 py-2 text-sm transition-colors', !vistaGrid ? 'bg-primary-600 text-white' : 'hover:bg-(--bg-hover)']"
                :style="vistaGrid ? { color: 'var(--text-muted)' } : {}"
                aria-label="Vista tabla"
                @click="vistaGrid = false"
              >
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 10h16M4 14h16M4 18h16" />
                </svg>
              </button>
              <button
                :class="['px-3 py-2 text-sm transition-colors', vistaGrid ? 'bg-primary-600 text-white' : 'hover:bg-(--bg-hover)']"
                :style="!vistaGrid ? { color: 'var(--text-muted)' } : {}"
                aria-label="Vista tarjetas"
                @click="vistaGrid = true"
              >
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" />
                </svg>
              </button>
            </div>
          </template>
        </SearchToolbar>
      </template>

      <template #content>
        <AppCard fill-height padding="none">
          <template #header>
            <div class="px-6 py-4 border-b shrink-0" style="border-color: var(--border-color)">
              <h2 class="font-semibold" style="color:var(--text-primary)">
                {{ mascotasStore.totalElements }} mascota(s)
              </h2>
            </div>
          </template>

          <!-- TABLA -->
          <div v-if="!vistaGrid">
            <AppTable
              :columns="columns"
              :rows="mascotasStore.mascotas"
              :loading="mascotasStore.loading"
              empty-message="No se encontraron mascotas"
              @row-click="abrirEditar"
            >
              <template #cell-mascota="{ row }">
                <div class="flex items-center gap-3">
                  <MascotaAvatar :especie-nombre="row.especie?.nombre ?? ''" :nombre="row.nombre" size="sm" />
                  <div>
                    <p class="font-medium" style="color:var(--text-primary)">{{ row.nombre }}</p>
                    <p class="text-xs" style="color:var(--text-muted)">{{ row.clienteNombre }}</p>
                  </div>
                </div>
              </template>
              <template #cell-especie_raza="{ row }">
                <p style="color:var(--text-secondary)">{{ row.especie?.nombre ?? '-' }}</p>
                <p class="text-xs" style="color:var(--text-muted)">{{ row.raza?.nombre ?? '-' }}</p>
              </template>
              <template #cell-sexo="{ row }">
                <AppBadge :variant="row.sexo === 'macho' ? 'info' : 'warning'" size="sm">
                  {{ row.sexo === 'macho' ? '♂ Macho' : '♀ Hembra' }}
                </AppBadge>
              </template>
              <template #cell-edad="{ row }">{{ calcularEdad(row.fechaNacimiento) }}</template>
              <template #cell-peso="{ row }">{{ row.peso ? row.peso + ' kg' : '-' }}</template>
              <template #cell-propietario="{ row }">{{ row.clienteNombre }}</template>
              <template #actions="{ row }">
                <AppButton variant="ghost" size="sm" title="Editar mascota" @click.stop="abrirEditar(row)">
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                      d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                  </svg>
                </AppButton>
              </template>
            </AppTable>
          </div>

          <!-- GRID -->
          <div v-else class="p-4 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            <EntityCard
              v-for="mascota in mascotasStore.mascotas"
              :key="mascota.id"
              :title="mascota.nombre"
              :subtitle="`${mascota.especie?.nombre ?? '-'} - ${mascota.raza?.nombre ?? '-'}`"
              :status="{ label: mascota.estado ? 'Activo' : 'Inactivo', variant: mascota.estado ? 'success' : 'danger', dot: true }"
            >
              <template #avatar>
                <MascotaAvatar :especie-nombre="mascota.especie?.nombre ?? ''" :nombre="mascota.nombre" size="md" />
              </template>
              <template #details>
                <div class="space-y-1 text-sm">
                  <p style="color:var(--text-secondary)">Sexo: <strong>{{ mascota.sexo }}</strong></p>
                  <p style="color:var(--text-secondary)">Edad: <strong>{{ calcularEdad(mascota.fechaNacimiento) }}</strong></p>
                  <p style="color:var(--text-secondary)">Peso: <strong>{{ mascota.peso ? mascota.peso + ' kg' : '-' }}</strong></p>
                  <p style="color:var(--text-secondary)">Propietario: <strong>{{ mascota.clienteNombre }}</strong></p>
                </div>
              </template>
              <template #actions>
                <AppButton variant="ghost" size="sm" @click="abrirEditar(mascota)">
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                      d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                  </svg>
                </AppButton>
              </template>
            </EntityCard>
            <EmptyState
              v-if="mascotasStore.mascotas.length === 0 && !mascotasStore.loading"
              title="No se encontraron mascotas"
              message="Intenta con otro filtro o registra una nueva mascota"
            />
          </div>

          <template #footer>
            <div class="border-t" style="border-color:var(--border-color)">
              <AppPagination
                :page="mascotasStore.page"
                :total-pages="mascotasStore.totalPages"
                :total-elements="mascotasStore.totalElements"
                :page-size="mascotasStore.pageSize"
                :loading="mascotasStore.loading"
                @change="mascotasStore.irAPagina"
              />
            </div>
          </template>
        </AppCard>
      </template>
    </TableViewLayout>

    <!-- MODAL -->
    <AppModal
      v-model="showModal"
      :title="mascotaEditando ? 'Editar mascota' : 'Nueva mascota'"
      size="lg"
    >
      <MascotaForm
        :mascota="mascotaEditando"
        :especies="mascotasStore.especies"
        :razas="mascotasStore.razas"
        :clientes="clientesDropdown"
        :loading="loading"
        @submit="handleSubmit"
        @cancel="showModal = false"
      />
    </AppModal>
  </DashboardLayout>
</template>